...
def geturls():
    ...
    sectionTitles = driver.find_elements_by_css_selector('.section-title .section-lock')
    for course in driver.find_elements_by_css_selector('.section-list a.item'):
        try:
            ...
            print('title: ' + title)
            downloadVids(driver.find_element_by_css_selector('.download').get_attribute('href'), 'node/' + title + '.mp4')
            print('ok')
        except:
            print('not found')
            pass
